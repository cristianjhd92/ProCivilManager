...
// Description: Ruta protegida genérica. Verifica que exista un token y un    // Descripción corta del propósito del archivo.
//              objeto "user" válido en localStorage antes de permitir el     // Asegura autenticación básica antes de mostrar rutas internas.
//              acceso. Si la sesión es inválida o está corrupta, limpia      // Limpia token y user si están ausentes o dañados.
//              los datos y redirige al inicio de sesión, disparando además    // Lanza un mensaje global de sesión expirada usando el contexto.
//              un mensaje global en el contexto de sesión para mostrar un    // Permite que BannerMensajeSesion muestre avisos al usuario.
//              aviso del tipo "Tu sesión expiró" mediante BannerMensajeSesion.//
...
//   - Disparar el mensaje global de "sesión expirada" usando el contexto.
//   - Redirigir al usuario a la ruta de inicio de sesión ("/login").
// Esto permite reutilizar el patrón mensaje + navegación de forma limpia.
